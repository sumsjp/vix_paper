...
#### **(2) 研究貢獻**
- **驗證了 VIX 期限結構的預測能力**，填補了市場預測研究的空白。
- **提出 C-MVO 策略並證明其優勢**，為量化交易提供新方法。
- **比較不同機器學習模型，發現線性回歸仍具競爭力**，對學術界和投資界均有參考價值。

### **6. 未來研究方向**
    - **使用高頻數據（High-Frequency Data）提升預測準確度**  
    - **探索更複雜的深度學習架構（如 Transformer, Graph Neural Network）**  
    - **加入更多市場因子（如宏觀經濟指標、流動性變數）**  
    - **擴展研究範圍，測試策略在不同市場條件下的適應性**  

#### **總結**
本研究證明 **VIX CMFs 期限結構可作為有效的交易信號**，並且 **C-MVO 策略能顯著提高收益與風險控制能力**。這為量化交易、波動率預測與機器學習應用提供了重要參考。
